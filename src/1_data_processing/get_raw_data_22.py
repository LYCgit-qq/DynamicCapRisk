...
print("正在生成22维特征并按模态拆分...")
new_act_list = []  # 67个样本，每个(T, 10) - 操纵行为+车辆响应
new_eye_list = []  # 67个样本，每个(T, 6)  - 眼动认知
new_phy_list = []  # 67个样本，每个(T, 6)  - 生理状态
...
for i in range(len(act_list)):
    ...
    # [4] 瞳孔直径（数据增强）
    np.random.seed(i)
    new_eye[:, 4] = np.clip(np.random.normal(3.0, 0.5, T_act), 2.0, 4.0)
    # [5] 注视点分散度
    new_eye[:, 5] = calculate_gaze_dispersion(eye_60[:, 1:3])
    
    # ====================== 2.4 构建新的 phy (6维) ======================
    # 对应表3.6最后6个特征
    new_phy = np.zeros((T_act, 6))
    ...
    new_phy[:, 5] = np.clip(np.random.normal(5.0, 1.0, T_act), 3.0, 7.0)
...
output_data = {
    ...
    "eye": new_eye_list,  # 67个样本，每个(T, 6)
    "phy": new_phy_list   # 67个样本，每个(T, 6)
}
...
print(" 【eye  6维】: 0-注视X, 1-注视Y, 2-眨眼频率, 3-眨眼标准差, 4-瞳孔直径, 5-分散度")
print(" 【phy  6维】: 0-心率均值, 1-HRV, 2-BVP, 3-ECG, 4-RESP, 5-皮肤电导")
print("="*60)