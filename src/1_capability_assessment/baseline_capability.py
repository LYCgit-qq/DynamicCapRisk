...
def merge_configs(
    default_config: Dict[str, Any], cli_args: argparse.Namespace
) -> Dict[str, Any]:
...
def find_best_k(
    X: pd.DataFrame, output_dir: Path, config: Dict[str, Any]
...
def quantify_benchmark_ability(
    centers: pd.DataFrame, config: Dict[str, Any]
) -> pd.DataFrame:
...
def compute_individualized_abc(
    ...
    group_order = sorted(
        subj_df["能力等级"].unique(), key=lambda g: group_ab.get(g, 0.75), reverse=True
    )
...
def evaluate_benchmark_driving_ability(config: Dict[str, Any]):
    ...
    benchmark_result = quantify_benchmark_ability(
        centers.drop("综合得分", axis=1), config
    )
    ...
    # 8. 保存结果
    labels.to_csv(
        res_dir / "Ab_cluster_labels.csv", encoding="utf-8-sig", header=["能力等级"]
    )
    centers.to_csv(
        res_dir / "Ab_cluster_centers_all_indicators.csv", encoding="utf-8-sig"
    )
    ...
    abc_df.to_csv(
        res_dir / "Abc_individualized_baseline_ability.csv",
        index=False,
        encoding="utf-8-sig",
    )
...
def main():
    ...
    try:
        ...
        logging.info(
            "配置加载完成：聚类范围=%s，最佳k=%d，输入=%s，输出=%s",
            final_config["cluster_range"],
            final_config["best_k"],
            final_config["input_path"],
