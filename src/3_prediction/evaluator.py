...
from src.visualization.plot_prediction import (
    plot_training_loss,
    plot_risk_regression_6groups,
...
# 风险阈值加载函数
# =============================================================================
def load_risk_thresholds(cfg: dict) -> Tuple[float, float]:
    eval_cfg = cfg["eval"]
    source = eval_cfg.get("threshold_source", "config").lower()

    if source == "file":
        csv_path = eval_cfg["threshold_csv_path"]
        if not os.path.exists(csv_path):
            raise FileNotFoundError(f"阈值文件不存在: {csv_path}")

        df = pd.read_csv(csv_path, encoding="utf-8")
        low_thresh = df[df["指标"] == "低风险阈值θ1"]["数值"].values[0]
        high_thresh = df[df["指标"] == "高风险阈值θ2"]["数值"].values[0]
        print(f"✅ 从CSV文件加载阈值：低风险={low_thresh:.2f}，高风险={high_thresh:.2f}")
    else:
        low_thresh = eval_cfg["risk_thresh_low"]
        high_thresh = eval_cfg["risk_thresh_high"]
        print(f"✅ 从配置文件加载阈值：低风险={low_thresh:.2f}，高风险={high_thresh:.2f}")
...
def build_report(
    ...
    cfg:   dict,
    high_thresh: float
) -> Tuple[str, Dict, pd.DataFrame, np.ndarray]:
    ec     = cfg["eval"]
    ...
    hr_recall, hr_prec = high_risk_metrics(
        preds["risk_reg_true"], preds["risk_reg_pred"], high_thresh
    )
...
# =============================================================================
# ✅ 核心修改：evaluate 函数仅接收 cfg，无 args 参数
# =============================================================================
def evaluate(cfg: dict) -> None:
    ...
    low_thresh, high_thresh = load_risk_thresholds(cfg)

    print(f"\n[1/5] 加载模型检查点: {ckpt_path}")
    ...
    print(f"\n[2/5] 加载数据集: {cfg['paths']['dataset_pkl']}")
    ...
    print("\n[4/5] 计算评估指标...")
    report_str, metrics, pred_df, cm = build_report(preds, cfg, high_thresh)
    ...
    if cfg["eval"]["visualize"]:
        ...
        risk_path = os.path.join(out_dir, "risk_regression_6groups.png")
        ...
        try:
            true_risk = preds["risk_reg_true"]
            pred_risk = preds["risk_reg_pred"]
            total_samples = len(true_risk)

            group_size = max(50, total_samples // 6)
            group_data_real = []
            for i in range(6):
                start = i * group_size
                end = start + group_size
                slice_true = true_risk[start:end]
                slice_pred = pred_risk[start:end]
                if len(slice_true) == 0:
                    break
                group_data_real.append({"true": slice_true, "pred": slice_pred})

            plot_risk_regression_6groups(group_data_real, risk_path, low_thresh=low_thresh, high_thresh=high_thresh)
            print(f"✅ 6组风险度预测图绘制完成")
        except Exception as e:
            print(f"⚠ 风险度回归图绘制失败: {e}")

        try:
            plot_confusion_matrix(cm, cfg["class_names"], cm_path)
            print(f"✅ 混淆矩阵绘制完成")
        except Exception as e:
            print(f"⚠ 混淆矩阵绘制失败: {e}")

# =============================================================================
# 命令行入口：参数直接修改 cfg，不传递给 evaluate
# =============================================================================
def main():
    ...
    parser.add_argument("--no-visualize", action="store_true",
                        help="关闭可视化绘图（覆盖yaml配置）")
    args = parser.parse_args()

    cfg = load_config(args.config)
    # 命令行参数直接修改 cfg 配置
    if args.ckpt:
        cfg["paths"]["ckpt"] = args.ckpt
    if args.split:
        cfg["eval"]["split"] = args.split
    if args.output_dir:
        cfg["paths"]["output_dir"] = args.output_dir
    # --no-visualize 直接关闭 cfg 中的可视化开关
