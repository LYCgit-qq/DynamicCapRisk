...
for i in range(len(act_list)):
    ...
    new_eye[:, 2] = eye[:, 0]
    new_eye[:, 3] = eye[:, 0]
    new_eye[:, 4] = calculate_gaze_dispersion(eye[:, 1:3])

    # ========== 构建 phy (5维 | 100Hz 原始长度) ==========
    T_phy = phy.shape[0]
    new_phy = np.zeros((T_phy, 5))
    new_phy[:, 0] = phy[:, 3]
    new_phy[:, 1] = np.abs(phy[:, 4])
    new_phy[:, 2] = phy[:, 0]
    # ECG强力平滑（原始功能保留）
    new_phy[:, 3] = gaussian_filter1d(phy[:, 1], sigma=20)
    new_phy[:, 4] = phy[:, 2]
...
# ====================== 日志 ======================
print("\n" + "="*60)
print("✅ 20维特征完成 | 无重采样 | 原始频率保留")
print(f"act=60Hz | eye=120Hz | phy=100Hz")
print("【act 10维】方向盘转角/角速度/油门/刹车/车速/纵向加/横向加/偏移量/X/Y")
print("【eye 5维】注视X/Y/眨眼频率/眨眼标准差/注视分散度")
print("【phy 5维】心率/HRV/BVP/ECG(平滑)/呼吸")
print("="*60)